#!/usr/bin/env python3
"""
IT and Consultancy Code Discovery - using dim_tac_subcodes_ws table
"""
...
# Check what columns are in dim_tac_subcodes_ws
print("Checking dim_tac_subcodes_ws structure...")
sample = con.execute("SELECT * FROM dim_tac_subcodes_ws LIMIT 3").fetchdf()
...
# First check what worksheet names look like
print("\nChecking worksheet naming...")
ws_sample = con.execute("SELECT DISTINCT ws_key FROM dim_tac_subcodes_ws WHERE ws_key LIKE '%intang%' LIMIT 5").fetchall()
print(f"Intangible worksheet keys: {ws_sample}")
...
print(intangibles_codes.head(20).to_string(index=False))
if len(intangibles_codes) > 20:
    print(f"... and {len(intangibles_codes) - 20} more")
...
print("\nNext step: Once verified, we can run the full analysis!")
